...
class KalmanModel:
    ...
    def __init__(self, args, acceleration=False):
        ...
        self.q = .03#0.03
        self.r = .03#0.03
        self.data_fps = args.FPS ## different across datasets
    ...
    def PredictTraj(self, trajectory):
        ...
        x0, y0 = trajectory[0]   ### the dimension mismatch..
...
#         print(f'error is {np.sqrt((residual**2).sum())}')
        ...
        xhat_k_new = xhat_k_minus + np.dot(self.K, residual) # predict future position
        self.P = np.dot((np.eye(self.n_dim) - np.dot(self.K, self.H)), P_minus) # update covariance
